...
// Classifies how a pair of numbers relate, reusing the same
// MOOLANK_COMPATIBILITY table (checked in both directions) so there's
// exactly one place to fine-tune all compatibility logic in the app.
export function classifyPair(a, b) {
  if (a === b) return "Repeating";
  const tableA = MOOLANK_COMPATIBILITY[a];
  const tableB = MOOLANK_COMPATIBILITY[b];
  const friendly = (tableA && tableA.friendly.includes(b)) || (tableB && tableB.friendly.includes(a));
  const enemy = (tableA && tableA.enemy.includes(b)) || (tableB && tableB.enemy.includes(a));
  if (friendly && !enemy) return "Benefic";
  if (enemy && !friendly) return "Malefic";
  return "Neutral";
}

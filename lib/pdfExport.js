...
export async function exportBirthReportPDF(report, options = {}) {
  ...
  ctx.ensureSpace(20);
  ctx.y = sectionTitle(doc, "Number Compatibility Table", ctx.y, bodyFont);
  doc.setFontSize(9);
  [1, 2, 3, 4, 5, 6, 7, 8, 9].forEach((n) => {
    ctx.ensureSpace(10);
    const status = translateCompat(getCompatibility(n, moolank));
    ctx.y = bodyText(doc, `${n} (${status}): ${(NUMBER_MEANINGS_I18N[lang] || NUMBER_MEANINGS_I18N.en)[n]}`, ctx.y);
    ctx.y += 2;
  });
  ctx.y += 4;
  ...
  const { calcUniqueCombinationsFromDOB, calcRepeatingDigits } = await import("./numerology");
  const { classifyPair, combinationMeaning, REPEATING_NUMBER_TEXT, AFFIRMATIONS } = await import("./numerologyKnowledge");

  const repeatingDigits = calcRepeatingDigits(inputs.dob);
  if (repeatingDigits.length > 0) {
    ctx.ensureSpace(20);
    ctx.y = sectionTitle(doc, "Numbers Appearing More Than Once", ctx.y, bodyFont);
    doc.setFontSize(9);
    repeatingDigits.forEach((n) => {
      ctx.ensureSpace(14);
      ctx.y = bodyText(doc, `${n}: ${REPEATING_NUMBER_TEXT[n]}`, ctx.y);
      ctx.y += 3;
    });
  }

  const chartCombos = calcUniqueCombinationsFromDOB(inputs.dob, 2);
  if (chartCombos.length > 0) {
    ctx.ensureSpace(20);
    ctx.y = sectionTitle(doc, "Your Chart Shows Combinations Of", ctx.y, bodyFont);
    doc.setFontSize(9);
    chartCombos.forEach((pair) => {
      ctx.ensureSpace(16);
      doc.setFont(bodyFont, "bold");
      doc.text(`${pair.join(", ")} (${classifyPair(pair[0], pair[1])}):`, 14, ctx.y);
      ctx.y += 5;
      doc.setFont(bodyFont, "normal");
      ctx.y = bodyText(doc, combinationMeaning(pair), ctx.y);
      ctx.y += 3;
    });
  }

  if (options.areaOfStruggle?.length > 0) {
    ctx.ensureSpace(20);
    ctx.y = sectionTitle(doc, "Affirmations", ctx.y, bodyFont);
    doc.setFontSize(9);
    options.areaOfStruggle.forEach((area) => {
      if (!AFFIRMATIONS[area]) return;
      ctx.ensureSpace(20);
      doc.setFont(bodyFont, "bold");
      doc.text(area, 14, ctx.y);
      ctx.y += 5;
      doc.setFont(bodyFont, "normal");
      ctx.y = bodyText(doc, AFFIRMATIONS[area], ctx.y);
      ctx.y += 4;
    });
  }
...
export async function exportMobileReportPDF(report, options = {}) {
  ...
  const adjacentPairsForWarning = calcMobileAdjacentPairs(mobile);
  const maleficPairs = adjacentPairsForWarning.filter(([a, b]) => classifyPair(a, b) === "Malefic");
  const isUnluckyTotal = MOOLANK_COMPATIBILITY[moolank]?.enemy?.includes(grid.mobileTotal);
  if (isUnluckyTotal || maleficPairs.length > 0) {
    ctx.ensureSpace(20);
    doc.setTextColor(168, 72, 60);
    doc.setFont(bodyFont, "bold");
    doc.setFontSize(11);
    doc.text("This Mobile Number Is Not Recommended", 14, ctx.y);
    ctx.y += 6;
    doc.setFont(bodyFont, "normal");
    doc.setFontSize(9);
    let reason = "";
    if (isUnluckyTotal) reason += `The Mobile Total (${grid.mobileTotal}) is unlucky for Moolank ${moolank}. `;
    if (maleficPairs.length > 0) reason += `Contains Malefic combinations: ${maleficPairs.map(([a, b]) => `${a}${b}`).join(", ")}.`;
    ctx.y = bodyText(doc, reason, ctx.y);
    doc.setTextColor(0, 0, 0);
    ctx.y += 6;
  }
  ...
  const { calcRepeatingDigits } = await import("./numerology");
  const { REPEATING_NUMBER_TEXT } = await import("./numerologyKnowledge");
  const repeatingMobileDigits = calcRepeatingDigits(mobile);
  if (repeatingMobileDigits.length > 0) {
    ctx.ensureSpace(20);
    ctx.y = sectionTitle(doc, "Numbers Appearing More Than Once", ctx.y, bodyFont);
    doc.setFontSize(9);
    repeatingMobileDigits.forEach((n) => {
      ctx.ensureSpace(14);
      ctx.y = bodyText(doc, `${n}: ${REPEATING_NUMBER_TEXT[n]}`, ctx.y);
      ctx.y += 3;
    });
